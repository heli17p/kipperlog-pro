...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (activePoint) {
      const ap = activePoint as { p: LoadingPoint, s: Site };
      if (isInLoadingZoneRef.current !== ap.p.id) {
        if (Date.now() - lastManualResetTimeRef.current > RESET_COOLDOWN_MS) {
          isInLoadingZoneRef.current = ap.p.id;
          if (hasCanceledCurrentZoneRef.current !== ap.p.id) {
            detectionStartTimeRef.current = Date.now();
            detectionTimeoutRef.current = setTimeout(() => {
              const now = Date.now();
              const lastAutoLoad = state.loads
                .filter(l => l.isAutoGenerated && l.loadingPointId === ap.p.id)
                .sort((a,b) => b.timestamp - a.timestamp)[0];
              if (!lastAutoLoad || (now - lastAutoLoad.timestamp > 15000)) { 
                setState(prev => ({ ...prev, activeSiteId: ap.s.id }));
                const id = crypto.randomUUID();
                const volume = state.currentTruckType === TruckType.AXLE_3 ? 10 : 12;
                const newLoad: Load = {
                  id, siteId: ap.s.id, loadingPointId: ap.p.id,
                  material: ap.p.material, volume, truckType: state.currentTruckType,

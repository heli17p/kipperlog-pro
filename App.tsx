...
import { Icons, MATERIAL_COLORS } from './constants';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      // Fixed: Property 'removeLoadingStatus' does not exist on type 'Window & typeof globalThis'.
  ...
  useEffect(() => {
    if (activeTab === 'track' && mapContainerRef.current && !mapRef.current) {
      ...
      // WICHTIG: Karte zwingen, sich an Containergröße anzupassen
      setTimeout(() => {
  ...
  useEffect(() => {
    ...
    let activePoint: { p: LoadingPoint, s: Site } | null = null;
    state.sites.forEach(s => {
      s.loadingPoints.forEach(p => {
        const dist = calculateDistance(currentCoords.lat, currentCoords.lon, p.latitude, p.longitude);
        if (dist < LOADING_ZONE_RADIUS) activePoint = { p, s };
      });
    });
    if (activePoint) {
      if (isInLoadingZoneRef.current !== activePoint.p.id) {
        if (Date.now() - lastManualResetTimeRef.current > RESET_COOLDOWN_MS) {
          isInLoadingZoneRef.current = activePoint.p.id;
          if (hasCanceledCurrentZoneRef.current !== activePoint.p.id) {
            detectionStartTimeRef.current = Date.now();
            detectionTimeoutRef.current = setTimeout(() => {
              const now = Date.now();
              const lastAutoLoad = state.loads
                .filter(l => l.isAutoGenerated && l.loadingPointId === activePoint!.p.id)
                .sort((a,b) => b.timestamp - a.timestamp)[0];
              if (!lastAutoLoad || (now - lastAutoLoad.timestamp > 15000)) { 
                setState(prev => ({ ...prev, activeSiteId: activePoint!.s.id }));
                const id = crypto.randomUUID();
                const volume = state.currentTruckType === TruckType.AXLE_3 ? 10 : 12;
                const newLoad: Load = {
                  id, siteId: activePoint.s.id, loadingPointId: activePoint.p.id,
                  material: activePoint.p.material, volume, truckType: state.currentTruckType,
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              {filteredHistoryLoads.map(load => (
                <div key={load.id} className="bg-white p-4 rounded-2xl shadow-sm flex justify-between items-center border border-slate-200">
                  <div className="flex-1">
                    <div className="flex gap-2 mb-1"><span className={`text-[8px] font-black text-white px-2 py-0.5 rounded uppercase ${getMaterialColor(load.material)}`}>{load.material}</span></div>
          ...
          <div className="p-4 space-y-4">
            ...
            {Object.entries(filteredStatsBySite).length > 0 ? (
              <div className="space-y-6 animate-in fade-in duration-300">
                {Object.entries(filteredStatsBySite).map(([siteId, siteData]) => (
                  <div key={siteId} className="bg-white rounded-[2rem] shadow-md overflow-hidden border border-slate-200">
                    ...
                    <div className="p-5 space-y-3">
                      {Object.entries(siteData.materials).map(([mat, data]) => data.count > 0 ? (
                    ...
                    <div className="bg-slate-50 p-4 text-right border-t border-slate-100"><span className="text-[10px] font-black text-slate-400 uppercase mr-2">Zeitraum Summe:</span><span className="text-xl font-black text-slate-900 italic">{Object.values(siteData.materials).reduce((s,d) => s+d.volume, 0).toFixed(1)} m³</span></div>
                  </div>
                ))}
                <div className="bg-amber-500 p-6 rounded-[2.5rem] text-white shadow-xl">
                    ...
                    <div className="flex justify-between items-end">
                        <span className="text-4xl font-black italic">{Object.values(filteredStatsBySite).reduce((acc, site) => acc + Object.values(site.materials).reduce((s, d) => s + d.volume, 0), 0).toFixed(1)} m³</span>
                        <span className="text-sm font-black uppercase opacity-80">{Object.values(filteredStatsBySite).reduce((acc, site) => acc + Object.values(site.materials).reduce((s, d) => s + d.count, 0), 0)} Fuhren</span>
                    </div>
                </div>
              </div>
            ) : (
